"""
run_updated.py — Full pipeline runner for stepmodelv2 with graph-conditioned stages.
...
Executes each stage in order:
  ...
  4. stage2_sft_with_graph.py — supervised fine-tune Qwen with LoRA + graph prefix
  5. stage3_grpo_with_graph.py — GRPO reinforcement learning with comprehensive reward
  6. comprehensive_evaluation.py — evaluate final model with comprehensive metrics
...
Usage:
    python run_updated.py                        # run all stages
    python run_updated.py --start-from stage2   # resume from a specific stage
    python run_updated.py --only generate_graphs build_input_json  # run specific stages

Available stage names:
    generate_graphs, build_input_json, stage1, stage2, stage3, evaluate, baseline_zeroshot, baseline_3shot, baseline_5shot, baseline_paper_cnn, comparison
"""
...
# Only stage2 and stage3 are changed to use graph-conditioned versions
STAGES = [
    ...
    ("stage2",            "training/stage2_sft_with_graph.py",   []),
    ("stage3",            "training/stage3_grpo_with_graph.py",    []),
    ("evaluate",          "eval/comprehensive_evaluation.py", []),
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the full stepmodelv2 pipeline with graph-conditioned stages.",
    ...
    print("\n" + "═" * 70)
    print("  stepmodelv2 — Full Pipeline Runner (Graph-Conditioned)")
    print("═" * 70)
